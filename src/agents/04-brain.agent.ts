  ...
  async execute(ctx: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      const { data: dataContext, issues: dataIssues } = await Promise.race([
        this.buildDataContext().then(result => {
          if (cancelled) return { data: 'No live data available.', issues: [] };
          return result;
        }),
        new Promise<{ data: string; issues: string[] }>(resolve => setTimeout(() => {
          dataTimedOut = true;
          cancelled = true; // signal buildDataContext to discard its result
      ...
      try {
        ...
        logger.warn(`[Brain] Knowledge base load failed: ${kErr.message}`);
      }
