...
// In-flight mutex for convertToLead — prevents two concurrent webhook/reply events
// for the same prospect from both creating LEAD_MASTER rows before either writes back.
const convertToLeadInFlight = new Set<string>();
...
export class CampaignBuilderAgent extends BaseAgent {
  ...
  public async handleWebhookEvent(eventType: string, prospectEmail: string): Promise<void> {
    ...
    // Named column indices for CAMPAIGN_SALES (0-based) — update here if sheet columns change
    const COL = {
      campaignId:        1,
      showName:          2,
      companyName:       3,
    ...
    try {
      ...
      const match = salesRows.slice(1)
        ...
        .find(({ row }) => (row[COL.contactEmail] || '').toLowerCase() === prospectEmail.toLowerCase());
      ...
      const showName    = row[COL.showName] || '';
      const contactName = row[COL.contactName] || '';
      const existingLeadMasterId = row[COL.leadMasterId] || '';
      const currentStatus = (row[COL.status] || '').toUpperCase();
      const now = new Date().toISOString();
      ...
      switch (eventType.toUpperCase()) {
        ...
        case 'INTERESTED':
          ...
          if (!existingLeadMasterId) {
            const collectedInfo: Record<string, string> = {
              standSize: row[COL.standSize] || '', budget: row[COL.budget] || '',
              showDates: row[COL.showDates] || '', phone: row[COL.phone] || '',
              requirements: row[COL.requirements] || '', website: row[COL.website] || '', logoUrl: row[COL.logoUrl] || '',
            };
  ...
  private async convertToLead(
    ...
    // Mutex: if another event for the same email is already converting, skip.
    const lockKey = contactEmail.toLowerCase();
    if (convertToLeadInFlight.has(lockKey)) {
      logger.warn(`[CampaignBuilder] convertToLead already in flight for ${contactEmail} — skipping duplicate`);
      return;
    }
    convertToLeadInFlight.add(lockKey);
    ...
    try {
      ...
      convertToLeadInFlight.delete(lockKey);
    }
  }
    ...
    for (const past of pastCampaigns) {
      try {
        ...
        if (stats.sent > 0) {
          ...
          if (replyRate >= 5 && details && details.emails?.length) {
